...
/**
 * A fragment representing a single NOT detail screen.
 * This fragment is either contained in a {@link Main}
 * in two-pane mode (on tablets) or a {@link OverviewDetailsActivity}
 * on handsets.
 */
public class OverviewDetailsFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View rootView = rootView = inflater.inflate(R.layout.detail_layout, container, false);
        ...
        if(args.containsKey("twopane")){
            if(args.getBoolean("twopane")){
                ViewGroup.LayoutParams params = kenburn.getLayoutParams();
                params.width = 250;
                kenburn.setLayoutParams(params);
            }
        }
        ...
        if (args.containsKey("WHERE")) {
            if (!args.getString("WHERE").equals("")) {
                ...
                if (c.getColumnName(4).equals("name")) {
                    ...
                    tv.setText(getString(R.string.recentlyChanged) + changed);

                    tv = (TextView) rootView.findViewById(R.id.detail_info2);
                    tv.setText(getString(R.string.didSelect) + (c.getInt(7) + 1) + getString(R.string.time_s));
        ...
        if (args.containsKey(("TWEET ID"))) {
            ...
            t = (TextView) rootView.findViewById(R.id.detail_info1);
            t.setText(getString(R.string.downloaded) + c.getString(3));
        ...
        if (args.containsKey("TWEET chapter")) {
            ...
            if (!chapter.equals("") && !section.equals("") && subchapter.equals("")) {
                ...
                if (c.getColumnName(3).equals("name")) {
                    ...
                    t = (TextView) rootView.findViewById(R.id.detail_info1);
                    t.setText(getString(R.string.findOriginals) + c.getString(4));
                }
        ...
        if (args.containsKey("§ ID")) {
            ...
            if (c.getString(1).equals("T")) {
                Cursor subdetail = db.select("SELECT * FROM SubDetail WHERE paragraph='" + c.getString(2) + "' AND sub='" + c.getString(3) + "'");
                LinearLayout ll = (LinearLayout) rootView.findViewById(R.id.detail_layout);
                int index = 5;

                while (subdetail.moveToNext()) {

                    LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);

                    TextView tn = new TextView(getActivity());
                    tn.setTextColor(getActivity().getResources().getColor(android.R.color.holo_red_dark));
                    tn.setTextSize(22);
                    tn.setText(subdetail.getString(1) + "." + subdetail.getString(2) + "." + subdetail.getString(3));
                    tn.setLayoutParams(lp);
                    tn.setGravity(Gravity.CENTER_HORIZONTAL);
                    tn.setPadding(8, 12, 8, 0);
                    tn.setTypeface(null, 1);
                    ...
                    t.setText("'" + subdetail.getString(4) + "'");
                    t.setLayoutParams(lp);
                    t.setGravity(Gravity.LEFT);
                    t.setPadding(8, 5, 8, 30);
